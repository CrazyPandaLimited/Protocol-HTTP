...
const std::string TEST_DIR = TEST_ROOT + "test-fragmented-method";

TEST_CASE("parsing message with fragmented method", "[fragmented]") {
    ...
    for (auto file_name : read_directory(TEST_DIR)) {
        ...
        request = request_parser.parse(buffer).request;
    }
